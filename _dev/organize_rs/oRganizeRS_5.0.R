createTemplates=function(
	...
	if(debug) print("DEBUG MODE: createTemplates")
	options(stringsAsFactors = F)
	if(!dir.exists(dir_projects)) stop("argument 'dir_projects' does not exist, check arguments and try again.")
	...
	if(!rescan & file.exists(templateRDS)){
		...
		if(!debug) proj_contents = lapply(dir_projects_in,function(x)try(.scan_1project(x)))
		if(debug) proj_contents = lapply(dir_projects_in[1:5],function(x)try(.scan_1project(x)))

		#create templates for generic reorganization
		proj_templates = lapply(proj_contents,function(x,y) try(.template_1project(x,y)), dir_out)
	...
	for(i in 1:length(proj_templates)){
		try(.write_1template(proj_templates[[i]]))
	}
...
moveRS=function(dir=NA,copy=T,cut=F,dir_out = NA){
	...
	if(!file.exists(this_template)){
		...
		#clean paths and track files that were copied
		all_files = tolower(.clean_path(list.files(dir_in,recursive=T, full.names=T)))
		...
		for(i in 1:nrow(template_in1)){
			...
			if(template_in1[i,"type"] != "other"){
				...
				files_this_B = grep(paste(gsub("[.]","[.]",template_in1[i,"type"]),".{0,1}$",sep="") , files_this_A , fixed=F , value=T )
				remaining_files = remaining_files[ ! tolower(remaining_files) %in% files_this_B ]
			...
			#actually move files
			lapply(unique(dirname(files_to)),function(x)if(!dir.exists(x))dir.create(x,recursive=T))
			if(copy){
				try( file.copy( files_this_B , files_to, overwrite = F) )
			}
			if(cut){
...
.write_1template=function(template_i){
	...
	#write csv files and batch files to projects
	dfi = template_i[["folders"]]

	if(nrow(dfi) > 0){

		dfi_files = template_i[["files"]]
		dir_i = template_i[["in_dir"]]
...
.fn_assign1=function(
	...
	if( length(folder_list) > 0 ){
		...
		if(type == "other"){
			#dir_new2 = .clean_path(paste(dir_new,subfolder,basename(folder_list_in),sep="/"))
			dir_new2 = dir_new1 = .clean_path(gsub(dir_old,paste(dir_new,"original",sep="/"),folder_list_in, fixed = T))
		}
...
if(F){
	...
	base_dir = "d:\\temp"
	copyFrom = file.path(base_dir,"test_copyFrom\\")
	copyTo = file.path(base_dir,"test_copyTo\\")
	createTemplates(copyFrom,copyTo, rescan = T)

	unlink(copyTo, recursive = T)
	moveRS(file.path(base_dir,"test_copyFrom\\New folder - Copy (3)"), copy = T, cut = F)
	moveRS(file.path(base_dir,"test_copyFrom\\New folder - Copy (3) - Copy"), copy = T, cut = F)
...
if(F){
	...
	base_dir = "d:\\temp"
	#path variables
	cutFrom = file.path(base_dir,"test_cutFrom")
	cutTo = file.path(base_dir,"test_cutTo")
	copyFrom = file.path(base_dir,"test_copyFrom")
