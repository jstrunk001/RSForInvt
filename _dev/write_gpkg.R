function (obj, dsn, layer, driver, dataset_options = NULL, layer_options = NULL,
          ...
          delete_dsn = FALSE, morphToESRI = NULL, encoding = NULL)
{
  ...
  known <- c("numeric", "character", "factor",
             "POSIXt", "integer", "logical")
  if (!all(dfcls %in% known))
    stop("Can't convert columns of class: ", paste(unique(dfcls[!dfcls %in%
                                                                  known]), collapse = ","), "; column names: ",
         paste(names(obj@data)[!dfcls %in% known], collapse = ","))
  ...
  if (!all(dftof %in% known))
    stop("Can't convert columns of type: ", paste(unique(dftof[!dftof %in%
                                                                 known]), collapse = ","), "; column names: ",
         paste(names(obj@data)[!dftof %in% known], collapse = ","))
    ...
    if (already_exists) {
      if (overwrite_layer) {
        if (strsplit(getGDALVersionInfo(), " ")[[1]][2] >=
            "2" && is_shpfile && file.info(dsn)$isdir) {
          odsn <- dsn
          dsn <- paste(dsn, "/", layer, ".shp",
                       sep = "")
          if (verbose)
            warning(dsn, " substituted for ", odsn,
                    " for layer deletion")
  ...
  for (i in 1:nf) {
    ...
    else if (dfcls[i] == "character" && dftof[i] ==
             "character") {
      ldata[[i]] <- slot(obj, "data")[[i]]
    ...
    if (!is.null(encoding)) {
      if (ogr_ftype[i] == 4L) {
        ldata[[i]] <- iconv(ldata[[i]], from = encoding,
                            to = "UTF-8")
      }
    }
  }
  ...
  pre <- list(obj, as.character(dsn), as.character(layer),
              driver, as.integer(nobj), nf, as.character(fld_names),
  ...
  if (any(NAs) && is_shpfile) {
    hits <- which(NAs)
    warning("field name", ifelse(length(hits) == 1,
                                 " ", "s "), paste(fld_names[hits], collapse = ", "),
            " changed by driver to: ", paste(attr(res,
                                                  "ofld_nms")[hits], collapse = ", "))
  }
  if (verbose) {
    res <- list(object_type = res, ofld_nms = attr(res, "ofld_nms"),
                output_dsn = dsn, output_layer = layer, output_diver = driver,
                output_n = nobj, output_nfields = nf, output_fields = fld_names,
                output_fclasses = ogr_ftype, dataset_options = dataset_options,
                layer_options = layer_options, morphToESRI = morphToESRI,
                FIDs = FIDs)
    return(res)
  }
}
