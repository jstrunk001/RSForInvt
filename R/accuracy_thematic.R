...
accuracy_thematic = function(
  ...
  unq_levels_in = unique(c(obs,pred))

  #make sure everything is a factor
  if(class(pred) == "factor") pred_in = pred else pred_in = factor(as.character(pred), levels = unq_levels_in, labels = unq_levels_in)
  if(class(obs) == "factor") obs_in = obs else obs_in = factor(as.character(obs), levels = unq_levels_in, labels = unq_levels_in)

  #balance sample on observed data - optional
    #some evaluation criteria are affected by balance
  if(do_balance[1]){
    idx_bal = .fn_balance_idx(obs=obs_in,balance_min=balance_min)
  ...
  res_in0$F1 = median(res_in0$caret$byClass[,"F1"],na.rm = T)
...
.fn_balance_idx=function(obs,balance_min){
    tb_in0 = table(obs)
